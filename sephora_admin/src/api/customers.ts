...
export const customersApi = {
  getAll: async (query = ""): Promise<{ results: Customer[] }> => {
    return fetchJSON(`/api/users/admin/customers/${query}`);
  },

  getOne: async (id: number): Promise<CustomerDetail> => {
    return fetchJSON(`/api/users/admin/customers/${id}/`);
  },
  ...
  toggle: async (
    ...
    return fetchJSON(`/api/users/admin/customers/${id}/toggle-block/`, {
      method: "PATCH",
